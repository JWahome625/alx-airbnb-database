...
INSERT INTO bookings (booking_id, property_id, user_id, start_date, end_date, total_price, status) VALUES
    ...
    (uuid_generate_v4(), 
     (SELECT property_id FROM properties WHERE name = 'Ruiru Studio Apartment'), 
     (SELECT user_id FROM users WHERE first_name = 'Leah'), 
     '2025-06-19', '2025-07-20', 10000.00, 'confirmed');

-- Insert payments
INSERT INTO payments (payment_id, booking_id, amount, payment_method) VALUES
    (uuid_generate_v4(), 
     (SELECT booking_id FROM bookings WHERE total_price = 14500.00), 
     14000, 'credit_card'),
    (uuid_generate_v4(), 
     (SELECT booking_id FROM bookings WHERE total_price = 10400.00), 
     10000.00, 'paypal');
